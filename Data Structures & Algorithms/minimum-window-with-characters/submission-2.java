class Solution {
    /*
    Time: O(n) where n is the length of s. Each character is visited by right once and by left at most once.
    Space: O(1). The freq array is fixed at size 128 regardless of input size.
    
    Sliding window over s. Expand right until window contains all characters from t. 
    Shrink left to remove unnecessary characters while keeping it valid. 
    Track the smallest window found. Return it.
    */
